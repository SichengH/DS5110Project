...
library(tidyverse)
library(reshape2)
load("~/Documents/Northeastern_University/DS5110_Data_Processing_Data_Management/DS5110Project/data.coord.rda")
points_new <- read_csv("data/points_new.csv", 
                       col_names = FALSE)
...
check_points_new2 <- points_new2 %>% group_by(PID) %>% 
  summarize(upid = n()) %>% group_by(upid) %>%
...
# data.coord%>%
#   filter(!PID %in% pid.remove)->data.coord.update

data.coord%>%
  filter(!PID %in% pid.remove)%>%
  left_join(y=notkicked,by = "PID")%>%
