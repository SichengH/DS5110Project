...
num_ac <- unique(data.coord$AC)
num_heat <- unique(data.coord$HEAT)
...
server <- function(input, output, session) { 
  ...
  output$boston_map <- renderLeaflet({
    ...
    addMarkers(data = sample, lng = ~LON, lat = ~LAT,
               ...
               group = "markers", popup = paste0("<strong>Property Info</strong><br>",
                                                 "<br>Year Built: ",sample$YR_BUILT,
                                                 "<br>Year Remodeled: ",sample$YR_REMO,
                                                 "<br>Full Address: ",sample$full_address,
                                                 "<br>Interior Score: ",
                                                 as.numeric(as.character(sample$INT_SCORE))^(5/2)))
  })
